...
export function meta( {
    ...
    return [
        ...
        {
            name: "description",
            content: "Single floating world interactive portfolio showcase" 
        }
    ];
}
...
export default function Home() {
    ...
    // Cinematic Opening Positions (Far away, high sky view)
        ...
        worldRotationY: 0,
        lerpSpeed: 0.035,
        travelCurve: null as THREE.QuadraticBezierCurve3 | null,
        travelProgress: 1,
        travelSpeed: 0.012
    } );
    ...
    const createTravelCurve = (
        ...
        const curveStrength = 2.5;

        const controlX = midX + dirX * curveStrength;
        const controlZ = midZ + dirZ * curveStrength;
    ...
    useEffect(
        () => {
            ...
            const target = MILESTONES[currentIndex].targetPos;

            state.travelCurve = createTravelCurve(
                state.hobbitX,
                state.hobbitZ,
                target.x,
                target.z
  ...
  state.travelProgress < 1
                    ...
                    state.travelProgress += state.travelSpeed;
                    ...
                    hobbitCharacter.position.set(
                        pos.x,
                        Math.abs( Math.sin( totalElapsedTime * 10 ) ) * 0.12,
                        pos.z
                    );

                    const tangent =
                        state.travelCurve.getTangent( t );

                    hobbitCharacter.rotation.y =
                        Math.atan2(
                            tangent.x,
                            tangent.z
                        );
                } else {
                    hobbitCharacter.position.y = 0;
                }
